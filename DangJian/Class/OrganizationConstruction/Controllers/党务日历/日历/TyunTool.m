...
@implementation TyunTool

+ (NSInteger)getNumberWithMonth:(NSString *)monthStirng
{
    NS_SUPPRESS_DECLARATIONS_USE(

    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
    [dateFormatter setDateFormat:@"yyyy/MM"];
    NSDate *date = [dateFormatter dateFromString:monthStirng];
...
+ (NSInteger)getWeekdayWithDate:(NSString *)date {
    ...
    _comps = [gregorian components:NSCalendarUnitWeekday fromDate:_date];
    
    NSInteger weekday =_comps.weekday;
 
    //注   系统返回的weekday为1~7  其中1代表周日 做处理用0表示周日
    return weekday - 1;
}
...
+(NSString *)LunarForSolar:(NSString *)dayString{
    ...
    NSDate *solarDate = [dateFormatter dateFromString:dayString];
  /**
    //天干名称
...
//    NSString *szNongli = [NSString stringWithFormat:@"%@(%@%@)年",szShuXiang, (NSString *)[cTianGan objectAtIndex:((wCurYear - 4) % 60) % 10],(NSString *)[cDiZhi objectAtIndex:((wCurYear - 4) % 60) % 12]];
    ...
    NSString *lunarDate = [NSString stringWithFormat:@"%@",(NSString *)[cDayName objectAtIndex:wCurDay]];
...
{
    NSArray *arr = [dodate componentsSeparatedByString:@" "];
    NSString *str = arr.firstObject;
    NSString *returnStr = [str stringByReplacingOccurrencesOfString:@"-" withString:@"/"];
