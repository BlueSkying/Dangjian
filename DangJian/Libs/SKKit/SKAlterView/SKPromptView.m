...
@interface SKPromptView ()<UITextFieldDelegate> {
...
- (UILabel *)titleLabel {
    if (!_titleLabel) {
        ...
        titleLabel.backgroundColor = Color_system_red;
...
//            }
            ...
            [topLine setFrame:CGRectMake(0, 0, CGRectGetWidth(button.frame), 1)];
            topLine.backgroundColor = SystemGraySeparatedLineColor.CGColor;
...
-(void)clickToCancelAndDete:(UIButton *)sender
{
    ...
    [self dissmiss];
    if (sender.tag == _buttonTitles.count - 1) {
        ...
        self.sk_promptViewBlokc ? self.sk_promptViewBlokc() : nil;
    }
}
...
- (void)show {
    ...
    __weak SKPromptView *weakSelf = self;
