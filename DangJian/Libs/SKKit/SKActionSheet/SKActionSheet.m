...
@interface SKActionSheet ()<UIGestureRecognizerDelegate> {
    UIWindow *window;
...
-(void)loadUI{
    ...
    if (Ids == 1) {
        ButtomView.backgroundColor = [UIColor colorWithRed:223.0f/255.0f green:226.0f/255.f blue:236.0f/255.0f alpha:1]; //[UIColor redColor];//
    }
    CGFloat height;
    UIFont *font = [UIFont systemFontOfSize:12.0f];
    CGSize size = [self markGetAuthenticSize:_AttachTitle Font:font MaxSize:KMaxSize];
    ...
    Cancebtn.titleLabel.font = [UIFont systemFontOfSize:17];
    [Cancebtn setTag:100];
