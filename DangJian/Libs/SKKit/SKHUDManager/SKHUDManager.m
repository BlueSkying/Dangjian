...
@interface SKHUDManager ()<UIGestureRecognizerDelegate> {
...
UIView *bottomView;
static SKHUDManager *hudManager = nil;
UIView *hudAddedView;
...
+ (void) showLoadingInView:(UIView *) view{
    ...
    //    hud.color = SystemGrayBackgroundColor;
    [view addSubview:hud];
    [hud show:YES];
//    [hudManager addGestureInView:view];
}
+ (void) showLoadingText:(NSString *)text inView:(UIView *) view{
    hudAddedView = view;
    [self shareManager];
    if (view == nil) {
        view = [[UIApplication sharedApplication] windows].lastObject;
...
+ (void) showLoadingText:(NSString *)text {
    ...
    [SKHUDManager hideAlert];
    [self showLoadingText:text inView:nil];
}
...
-(void)tapTheScreen{
    NSLog(@"点击屏幕");
    [hudManager hideBackView];
    [tap removeTarget:self action:@selector(tapTheScreen)];
    [SKHUDManager hideAlert];
}
#pragma mark - 解决手势冲突
